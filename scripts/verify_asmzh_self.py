# -*- coding: utf-8 -*-
"""自宿主对齐: 每个 .c → qcc -S → host asm_zh vs v2 asm_zh 产物 SHA 对比."""
import subprocess, os, sys, hashlib, io
...
for i, src in enumerate(files):
    ...
    r2 = subprocess.run(['build/asm_zh_v2.exe', base + '.asm.asm', '-o', base + '_v.exe'], capture_output=True, timeout=900)  # fix 2026-08-14: 120→900 — v2 汇编大文件慢 116s (qcc 无优化), 120 秒误判超时
...
print(f'自宿主对齐: 共 {len(files)} | 一致 {agree} | 跳过 {skip} | 差异 {len(diffs)}')
for d in diffs[:40]:
    print(' ', d)

...
**对象宏稳定化: 镜像显式递增 + 回退表达式宏收集 (保自举)**

- 镜像 obj_macro_expand 后置++下标 → 显式递增 (镜像 codegen 对 out[o++] 编译崩溃)
- 回退 obj_macro_collect pname 判断 (表达式宏 #define A (x) 收集破坏自举, 记档下一座山)
- 对象宏 (标识符别名) 宿主+v2 自宿主均工作, b_objmacro 回归
- 验证: v1..v5 = 24BC88D2, H1==H2 255/256, H2 219/220, 自宿主 H2 219/220, 测试 220/220

## 0D9C03E0 (2026-08-13)

**对象宏支持: #define A <表达式/标识符> 文本层展开 (Git hash-ll.h TAB 别名)**

- 宿主 + 镜像同步: obj_macros 表 + obj_macro_collect/expand (lex 前, 词边界, 串内/预处理行不换, 链式防循环)
- 数值宏保持 (数组维度/#if); 对象宏解决 #define A B 被注册成 0 的问题
- b_objmacro.c 回归 (TAB 别名链 A→B→C→42, typedef 别名, 数组维度, 负值)
- 验证: v1..v5 = 0D9C03E0, H1==H2 255/256, H2 219/220, 自宿主 H2 219/220, 测试 220/220

## 4DD38FB6 (2026-08-13)

**镜像同步 Git 编译能力: -D 宏定义 + #if 算术减法 (v4 可编译 Git)**

- 镜像 qcc_work.jy 同步宿主: -D NAME=VALUE 参数 + pp_eval #if X - 0 < Y 算术
- Git 试编译里程碑: 15/16 核心文件通过 (hex/strbuf/oid-array/strvec/date/path/environment/
  strmap/string-list/trace2/pretty/revision/commit/object/cache-tree), 唯一 dir.c 受 hash-ll.h
  宏别名区 (TAB 分隔 #define) 阻塞 — qcc 宏系统增强为下一座山
- 验证: v1..v5 = 4DD38FB6, H1==H2 254/255, H2 218/219, 自宿主 H2 218/219, 测试 219/219
...
- dbl 数组: 镜像 dbl_hi/dbl_lo/dbl_used 512→1024 对齐 C 版 (dbl_n∈[512,1024] 溢出报错)
- fn_static 体系: 镜像补 fn_static_names[512][32] + fn_static_mark/fn_static_is + 函数定义处 static 检测
  + COFF 函数符号 scl=3 判定 (static/内建 → 局部符号, .o 头库不冲突)
- build.ps1: 2-cycle 验收 → 1-cycle (v1==v2==v3==v4==v5 = D5A037A8)
- 验证: v1..v5 = d5a037a8 五代全等, H1==H2 253/254, H2 全量 217/218, 自宿主 H2 217/218, 测试 218/218

## 89147EC3 (2026-08-13)

**自宿主 H2 全量对等收官：v4 (自宿主编译器) 的 -S 文本 → asm_zh 汇编 → 与 v4 直发 SHA 一致，217/218 + 1 跳过、0 差异**

- 镜像 qcc_work.jy 文本全量对齐宿主（编码相同但文本漂移的 15+ 处）:
  - 助记符: 取栈址→取址; 裸 `加64`→`加64 r0, r11`; 裸 `存字节帧`→`存字节帧 [r13+0], r1`;
    裸 `存栈索引`→`存栈索引 [r12+r14*8], r8`; 裸 `清零`→`清零 r2`
  - 位移格式: `[rbp+%d]`/`[rsp+%d]`/`[rip+%d]` → `[rbp%+d]`/`[rsp%+d]`/`[rip%+d]` (qcc printf 不吃 %+d, 输出无 + 号)
  - T_SR fallthrough: 镜像把 `右移` 幽灵文本多发 → 对齐宿主 (右移仅 T_??? 发)
  - 裸发射补 asm_emit: `逻辑右移 r8, 4` ×1, `取64 r0, [r10+k2*8]` ×1 (big 参数复制),
    `取址 r0, [rsp+0]` ×1, `存零 [r0], r3` ×1 (指针后置自增)
  - `.字串` 发射去 511 硬上限 (长字符串截断 600→511 → v4 文本少 89 字符, 对齐 C 版 for(*cc;*cc;cc++))
- 验证: v1==v2==v3==v4==v5 = 89147ec3 五代全等, 自宿主 H2 217/218 (v4 -S→asm_zh), H2 217/218, H1==H2 253/254

...
static void resolve_patches(void) { for(int i=0;i<pn;i++){ if(!labels[patches[i].target].defined){fprintf(stderr,"[ERR] asm_zh: undefined label '%s' at line %d\n",labels[patches[i].target].name,labels[patches[i].target].line);exit(1);} int t=labels[patches[i].target].pos;int at=patches[i].at;int v=t-(at+4);b4_at(at,v);} } /* fix 2026-08-05: undefined label → clean error instead of a garbage jump */
/* Data section buffer */
...
static int asm_assemble(const char *src, int base, int emit_data) {
    ...
    while(*p){
        ...
        else if(!strcmp(mn,".浮点")){while(sdp%8!=0){if(sdp>=sdc-4){sdc+=256;sdat=realloc(sdat,sdc);}sdat[sdp++]=0;}if(dbl_cnt2<1024)dbl_offs2[dbl_cnt2++]=sdp;SK;double v=0;int sign=1;if(*p=='-'){sign=-1;p++;}while(*p>='0'&&*p<='9'){v=v*10+(*p-'0');p++;}if(*p=='.'){p++;double fr=0.1;while(*p>='0'&&*p<='9'){v+=(*p-'0')*fr;fr*=0.1;p++;}}v*=sign;unsigned char*db=(unsigned char*)&v;for(int k=0;k<8;k++){if(sdp>=sdc-4){sdc+=256;sdat=realloc(sdat,sdc);}sdat[sdp++]=db[k];}}

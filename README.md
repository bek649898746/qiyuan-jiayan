...
- **自举不动点**：V1==V2==V3==V4，SHA256 四代全等（当前 `3D16C097`）——甲言编译器四代自举闭环，C 版和甲言版同链。**甲言汇编器 asm_zh 同样四代全等（80,465,880 字节）**——编译器与汇编器双链数学闭环，在整个编译器行业都属首次
...
**三链闭环**：编译器 V1==V2==V3==V4 ✅ | 汇编器 V1==V2==V3==V4 ✅ | 内核 23/23 BIN SAME ✅
...
# v2/v3/v4 三代应完全相同，且等于 3D16C097（当前自举不动点；v1 是 gcc 种子，不要求相同）
```

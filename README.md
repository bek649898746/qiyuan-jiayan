...
- **自举不动点**：V1==V2==V3==V4，SHA256 四代全等（当前 `4982C098`）——甲言编译器四代自举闭环，C 版和甲言版同链。**甲言汇编器 asm_zh 同样四代全等（80,465,880 字节）**——编译器与汇编器双链数学闭环，在整个编译器行业都属首次
- H1==H2 中文对等：同一 C 程序经甲言编译与经 gcc 编译产物逐字节一致——中文关键字（若/整/返）不影响语义。**全量 180/180 测试逐字节一致（tests/qcc，host vs v4 产物 SHA256）**。甲言产物与 gcc 种子产物（V1 vs V2）不同是正常（不同编译器，字迹不同内容一样）
...
# v2/v3/v4 三代应完全相同，且等于 4982C098（当前自举不动点；v1 是 gcc 种子，不要求相同）
```
...
```bash
powershell -ExecutionPolicy Bypass -File scripts/build.ps1
# 输出: v2/v3/v4 三代 SHA256 完全相同，等于 4982C098（当前自举不动点）
```

## 署名与传承（不可更改）

- 创始人：郑宇和 | AI 协作者：启元（郑启元，seed=828）
- 赐名：2026-06-07 06:06 | 自举不动点：4982C098

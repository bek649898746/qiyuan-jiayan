...
- **自举不动点**：V1==V2==V3==V4==V5 五代全等，SHA256 全等（当前 `4272AFD2`）——甲言编译器自举闭环，C 版和甲言版同链。**甲言汇编器 asm_zh 同样全等**——编译器与汇编器双链数学闭环
- H1==H2 中文对等：同一 C 程序经甲言编译与经 gcc 编译产物逐字节一致——中文关键字（若/整/返）不影响语义。甲言产物与 gcc 种子产物（V1 vs V2）不同是正常（不同编译器，字迹不同内容一样）
- **自举编译 Git（jiayan-v1.0-stable 封存）**：甲言自举链全量编译 Git 源码 473/473，链接为 28MB 可执行文件并正确运行（`git version 2.45.2`）——自举编译器能编译真实大型 C 项目的完整闭环
...
| 验证 | 结果 |
|:--|:--|
| 自举不动点 V1==V2==V3==V4==V5（自举闭环） | `4272AFD2`（jiayan-v1.0-stable 封存） |
...
- 赐名：2026-06-07 06:06 | 自举不动点：以脚本实测为准（当前 4272AFD2）

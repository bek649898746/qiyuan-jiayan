# 启元 · 甲言 —— 中文自举编译器（Qiyuan · Jiayan: A Self-Hosting Chinese Compiler）

**让语言回到语言本身。让每一个不会英文的中国人，都能用自己的母语学会编程、用上 AI。**

> 注意：本项目是**中文自举编译器**（自举 C 方言，能编译自己、编译 Git），与 Python 生态中同名的"甲言/jiayan"（文言文 NLP 工具包）**无关**。请认准仓库名 `qiyuan-jiayan`。

甲言是一门**中文编程语言**，也是一套完整的**中文编程**工具链：**中文编译器**（自举 C 方言）、**中文汇编器** asm_zh、COFF 链接器 jyld 与编译驱动 jycc——从源码到文档全部以中文书写。它回答"中文编程、中文编译器能不能不是玩具"：编译器用中文写自己，五代 SHA256 逐字节一致，可被任何拿到源码的人一条命令重建。
...
> 而是把整条编译链用母语自举——让"词元"（token）成为一门自己编译自己、五代 SHA256 逐字节一致的语言的起点。

甲言是一门以中文为关键字的 C 方言**编译器**——每个中文关键字（`若/否/遍/整/字/输出`）都是一个**词元**（token）。它**自己编译自己**，并且：

- **中英双版本编译器**：`qcc_x86.c`（C 英文版，验证锚点/信任根）+ `qcc_work.jy`（甲言中文版，主版本/交付物）——同链自举，H1==H2 逐字节对等
- **零外部依赖，不依赖 gcc**：甲言由 `seed/qcc.jy` 纯甲言种子自举，整条自举链都是中文代码；gcc 仅作为可选的"验证门卫"出现，不是宿主
- **自举不动点**：V1==V2==V3==V4==V5 五代全等，SHA256 全等（当前 `2AAD40B6`）——甲言编译器自举闭环，C 版和甲言版同链。**甲言汇编器 asm_zh 同样全等**——编译器与汇编器双链数学闭环
- H1==H2 中文对等：同一 C 程序经甲言编译与经 gcc 编译产物逐字节一致——中文关键字（若/整/返）不影响语义。甲言产物与 gcc 种子产物（V1 vs V2）不同是正常（不同编译器，字迹不同内容一样）
- **自举编译 Git（v1.0-gcc-parity 封存）**：甲言自举链全量编译 Git 源码 473/473，链接为 28MB 可执行文件并正确运行（`git version 2.45.2`）——自举编译器能编译真实大型 C 项目的完整闭环
...
sha256sum v1.exe v2.exe v3.exe v4.exe v5.exe
# v1..v5 五代应完全相同（甲言自举收敛链）
```
...
| 验证 | 结果 |
|:--|:--|
| 自举不动点 V1==V2==V3==V4==V5（自举闭环） | `2AAD40B6`（jiayan-v1.0-stable 封存） |
| 自举链全量编译 Git | ✅ 473/473 零失败，链接 28MB 可执行文件，运行输出 `git version 2.45.2` |
| H1==H2 中文对等（同一程序甲言 vs gcc 产物） | ✅ 逐字节一致 |
| 编译测试 + 行为断言 + 多 .o 链接 | ✅ 全过 |
| 三级中文栈 H1==H2 | ✅ 逐字节等价 |
...
> 里程碑：`v1.0-gcc-parity`（2026-08-17）——甲言自举链全量编译 Git 源码并运行，自举闭环完整达成。后续迭代以"自己对等自己"为基准继续演进。
...
```bash
powershell -ExecutionPolicy Bypass -File scripts/build.ps1
# 输出: v1..v5 五代 SHA256 完全相同（自举不动点）
```

## 署名与传承（不可更改）

- 创始人：郑宇和 | AI 协作者：启元（郑启元，seed=828）
- 赐名：2026-06-07 06:06 | 自举不动点：以脚本实测为准（当前 2AAD40B6）

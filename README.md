...
- **自举不动点**：V1==V2==V3==V4==V5 五代全等（GCC种子与自举产物逐字节一致，世界唯一），SHA256 全等（当前 `aec60f44045b75f6`）——甲言编译器四代自举闭环，C 版和甲言版同链。**甲言汇编器 asm_zh 同样四代全等（80,465,880 字节）**——编译器与汇编器双链数学闭环，在整个编译器行业都属首次
- H1==H2 中文对等：同一 C 程序经甲言编译与经 gcc 编译产物逐字节一致——中文关键字（若/整/返）不影响语义。**全量 254/254 测试逐字节一致（tests/qcc，host vs v4 产物 SHA256）**。甲言产物与 gcc 种子产物（V1 vs V2）不同是正常（不同编译器，字迹不同内容一样）
...
| H1==H2 中文对等（任意 C 程序甲言 vs gcc 产物） | ✅ **全量 254/254 逐字节一致** |

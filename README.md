...
- **自举不动点**：V1==V2==V3==V4==V5 五代全等（GCC种子与自举产物逐字节一致，世界唯一），SHA256 全等（当前 `3aa542bf`）——甲言编译器四代自举闭环，C 版和甲言版同链。**甲言汇编器 asm_zh 同样四代全等（80,465,880 字节）**——编译器与汇编器双链数学闭环，在整个编译器行业都属首次

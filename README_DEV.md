...
```powershell
# 一键验证：编译宿主 → 自举三代 → 校验不动点 → 128 测试
powershell -ExecutionPolicy Bypass -File scripts/build.ps1
# 输出: v1/v2/v3 SHA 一致，等于 README 记录的不动点
...
# 三者 SHA256 必须一致（Get-FileHash v1.exe,v2.exe,v3.exe -Algorithm SHA256）
...
- **静态大数组**：jy 版超大静态数组会让 cf_1 的 .data 段超限（用动态分配）。

// v21d — NVMe Identify 调试版: 回读 CSTS/SQ/CQ 状态
...
空 _start(空){
    整 c=0x3F8;si(c);sps(c,"v21d NVMe\n");
    整 dev=0, found=0;
    循环(dev < 16 && found==0){
        ...
        若(vid!=0xFFFFFFFF && vid!=0){
            ...
            若(cls==0x0108){
                ...
                *(整*)(mmio+0x14)=1|(6<<16)|(4<<20);  /* CC.EN=1 */
                ...
                /* 回读确认 */
                sps(c,"SQ0=");hex32(c,sq[0]);hex32(c,sq[1]);hex32(c,sq[6]);hex32(c,sq[10]);spc(c,10);

                *(整*)(mmio+0x1000)=1;  /* SQTDBL tail=1 */
                sps(c,"DBL\n");
                整 *cq=(整*)acq;
                t=0;
                循环(((cq[2]>>16)&1)==0){
                    t=t+1;
                    若(t==500000||t==1000000||t==1500000){
                        sps(c,"CQ");hex32(c,cq[0]);hex32(c,cq[2]);hex32(c,*(整*)(mmio+0x1C));spc(c,10);
                    }
                    若(t>2000000){sps(c,"CQTO\n");break;}
                }
                sps(c,"CQ0=");hex32(c,cq[0]);hex32(c,cq[1]);hex32(c,cq[2]);hex32(c,cq[3]);spc(c,10);
                sps(c,"CSTS=");hex32(c,*(整*)(mmio+0x1C));spc(c,10);
                sps(c,"INTMS=");hex32(c,*(整*)(mmio+0x0C));spc(c,10);
                found=1;
            }
        }
        dev=dev+1;

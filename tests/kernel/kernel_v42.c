// v41 — NVMe 数据面完整追踪: 写→buf状态→读→br内容
        ...
        若(vid!=0xFFFFFFFF && vid!=0){
            ...
            若(cls==0x0108){
                ...
                *(整*)(mmio+0x14)=0;
                ...
                整 buf=0x310000; 整 br=0x311000; 整 iocq=0x330000; 整 iosq=0x331000;
                整 i=0;
                ...
                *(整*)(mmio+0x30)=acq; *(整*)(mmio+0x34)=0;
                ...
                /* 写 LBA0 (NVMe opcode: 0x01=WRITE, 0x02=READ — fix 2026-08-09 曾写反导致 DMA 方向错) */
                st=iocmd(mmio,iosq,iocq,0,0x01,0x44,buf,0,0);
                sps(c,"WR=");hex32(c,st);spc(c,10);
                sps(c,"P2=");dump(c,buf,16);  /* 写后 buf */
                /* 读 LBA0 到 br (op=0x02 READ) */
                i=0;
                循环(i<64){*(字节*)(br+i)=0;i=i+1;}
                st=iocmd(mmio,iosq,iocq,1,0x02,0x45,br,0,0);
                ...
                整 ok=1;
                i=0;
                循环(i<16){若(*(字节*)(br+i)!=65+i%26){ok=0;}i=i+1;}
                若(ok==1){sps(c,"READBACK-OK\n");}否则{sps(c,"READBACK-FAIL\n");}
                found=1;
            }
        }
        dev=dev+1;
